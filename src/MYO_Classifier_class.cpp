...
MYOClassifier::MYOClassifier(std::vector<std::string> training_files) { //, SparGlove& sg) {
	...
	for (int pose = 0; pose < training_files.size(); pose++) {
		...
		DataLogger::read_from_csv(training_file_data, training_files[pose]);
		//for (int i = 0; i < 7; i++) {
			/*read_in_object = std::vector<std::vector<double>>(200);
			for (int j = 0; j < 200; j++) {
				read_in_object[j] = file_data[i * 200 + j];
			}*/
		...
		for (int i = 0; i < 7; i++) {
			read_in_object[i] = std::vector<std::vector<double>>(200);
			for (int j = 0; j < 7; j++) {
				read_in_object[i][j] = std::vector<double >(200);
				for (int k = 0; k < 7; k++) {
					
					
					read_in_object[i][j][k] = training_files[f].read_from_csv();


					DataLogger::read_from_csv(read_in_object[i][j][k], v[f]);
				}
			}
		}*/
	}
	...
	//		std::cout << "Counter is " << std::endl;
		...
		if (Keyboard::is_key_pressed(Key::U)) {
			if (dir_classifier.is_trained()) {
				...
				DataLogger::read_from_csv(test_file_data, file_to_parse);
